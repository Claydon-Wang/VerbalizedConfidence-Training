...
## MATH (6 GPU config) 
# RLVR
#CUDA_VISIBLE_DEVICES=0,1,2,3,4,5 accelerate launch --num_processes 6 --config_file src/train/configs/launch/deepspeed.yaml -m src.train.train_main --dataset Math --method RLVR --model Qwen25_7B
# RLCR
#CUDA_VISIBLE_DEVICES=0,1,2,3,4,5 accelerate launch --num_processes 6 --config_file src/train/configs/launch/deepspeed.yaml -m src.train.train_main --dataset Math --method MathRLCR --model Qwen25_7B
# SFT+RLCR
#CUDA_VISIBLE_DEVICES=0,1,2,3,4,5 accelerate launch --num_processes 6 --config_file src/train/configs/launch/deepspeed.yaml -m src.train.train_main --dataset Math --method RLCRSFT --model Qwen25_7B
