...
class RLCRSplitCalTrainer(RLCRSplitTrainer):
    trainer_name = "rlcr_split_cal"

    def _compute_calibration_confidence_advantages(
        self,
        confidence_scores: torch.Tensor,
        group_success_rate: torch.Tensor,
    ):
        grouped_scores = confidence_scores.view(-1, self.num_generations)
        mean_grouped_scores = grouped_scores.mean(dim=1)
        std_grouped_scores = grouped_scores.std(dim=1)
        batch_accuracy = group_success_rate.mean()
        question_group_accuracy = group_success_rate.view(-1, self.num_generations).mean(dim=1)
        calibration_gaps = batch_accuracy - question_group_accuracy

        mean_grouped_scores = mean_grouped_scores.repeat_interleave(self.num_generations, dim=0)
        std_grouped_scores = std_grouped_scores.repeat_interleave(self.num_generations, dim=0)
        calibration_gaps = calibration_gaps.repeat_interleave(self.num_generations, dim=0)

        advantages = calibration_gaps * (confidence_scores - mean_grouped_scores) / (std_grouped_scores + 1e-4)
        return advantages, mean_grouped_scores, std_grouped_scores, calibration_gaps, batch_accuracy
    ...
    def _generate_and_score_completions(self, inputs, eval=False):
        ...
        confidence_reward_advantages, confidence_group_means, confidence_group_stds = self._normalize_group_rewards(
            reward_outputs["confidence_rewards"]
        )
        confidence_advantages, confidence_score_group_means, confidence_score_group_stds, calibration_gaps, batch_accuracy = (
            self._compute_calibration_confidence_advantages(
                reward_outputs["confidence_scores"],
                reward_outputs["group_success_rate"],
            )
        )
        ...
        self._metrics[mode]["reward_values/mean_group_confidence"].append(confidence_score_group_means.mean().item())
        self._metrics[mode]["reward_values/group_confidence_std"].append(confidence_score_group_stds.mean().item())
        self._metrics[mode]["reward_values/batch_accuracy"].append(batch_accuracy.item())
        self._metrics[mode]["reward_values/calibration_gap"].append(calibration_gaps.mean().item())
        self._metrics[mode]["reward_values/confidence_reward_advantage_mean_abs"].append(
            confidence_reward_advantages.abs().mean().item()
        )
