...
class RLCRSplitBatchTrainer(RLCRSplitTrainer):
    trainer_name = "rlcr_split_batch"

    confidence_std_floor = 0.02
    confidence_adv_eps = 1e-4

    def _compute_batch_confidence_advantages(self, confidence_rewards: torch.Tensor):
        batch_mean = confidence_rewards.mean()
        batch_sq_mean = confidence_rewards.square().mean()
        batch_var = torch.clamp(batch_sq_mean - batch_mean.square(), min=self.confidence_std_floor**2)
        batch_std = torch.sqrt(batch_var)
        confidence_advantages = (confidence_rewards - batch_mean) / (batch_std + self.confidence_adv_eps)
        return confidence_advantages, batch_mean, batch_std
    ...
    def _generate_and_score_completions(self, inputs, eval=False):
        ...
        confidence_advantages, confidence_batch_mean, confidence_batch_std = self._compute_batch_confidence_advantages(
        ...
        self._metrics[mode]["reward_total"].append(
            0.5 * (answer_group_means.mean().item() + confidence_batch_mean.item())
        )
        self._metrics[mode]["reward_std"].append(
            0.5 * (answer_group_stds.mean().item() + confidence_batch_std.item())
        )
