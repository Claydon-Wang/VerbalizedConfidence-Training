...
class RLCRSplitDATrainer(CoCADifficultyTrainer):
    trainer_name = "rlcr_split_da"
    ...
    def compute_rewards(self, generation_outputs, inputs):
        ...
        difficulty_targets = 1.0 - answer_rewards.view(-1, self.num_generations).mean(dim=1)
        difficulty_targets = difficulty_targets.repeat_interleave(self.num_generations, dim=0)
        difficulty_rewards = -torch.square(difficulty_scores - difficulty_targets)

        confidence_rewards_local = self.run_reward_function(
            "brier",
            prompts,
            completions,
            generation_outputs["completion_ids_list"],
        ...
        confidence_rewards_local = torch.as_tensor(confidence_rewards_local, dtype=torch.float32, device=device)
        confidence_rewards = gather(confidence_rewards_local)
